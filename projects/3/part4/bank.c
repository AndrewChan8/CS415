...
void initialize_puddles_bank(account *shared_memory, int num_accounts);
...
int main(int argc, char *argv[]){
  ...
  size_t shared_mem_size = num_accounts * sizeof(account);

  // Create shared memory for communication with Puddles Bank
  account *shared_memory = mmap(NULL, shared_mem_size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
  if (shared_memory == MAP_FAILED) {
    perror("mmap failed");
    exit(EXIT_FAILURE);
  }

  // Copy account data into shared memory
  memcpy(shared_memory, accounts, shared_mem_size);

  pid_t puddles_pid = fork();
  if (puddles_pid == -1) {
    perror("fork failed");
    exit(EXIT_FAILURE);
  } else if (puddles_pid == 0) {
    // Child process: Puddles Bank
    initialize_puddles_bank(shared_memory, num_accounts);
    exit(0); // Ensure the child process exits cleanly
  } else {
    // Parent process: Duck Bank
    // Continue with Duck Bank operations
  }
...
void initialize_puddles_bank(account *shared_memory, int num_accounts) {
  mkdir("savings", 0777); // Create savings directory
  for (int i = 0; i < num_accounts; i++) {
    double initial_savings_balance = shared_memory[i].balance * 0.2;
    double reward_rate = 0.02;

    // Write to savings/act_#.txt
    char savings_file[256];
    snprintf(savings_file, sizeof(savings_file), "savings/act_%d.txt", i);
    FILE *file = fopen(savings_file, "w");
    if (file) {
      fprintf(file, "Balance: %.2f\n", initial_savings_balance);
      fclose(file);
    } else {
      perror("Error creating savings file");
    }
  }
}
